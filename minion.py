...
class Source:
    ...
    # Regexes for the provider-agnostic context-limit probe. We send a
    # deliberately over-sized max_tokens with a one-token prompt so the server
    # tells us the limit without us ever generating. Different providers format
    # the rejection differently:
    #   • OpenAI / Together / Z.ai:
    ...
    _CTX_RANGE_RE = re.compile(r"valid range of max_tokens is \[\d+, (\d+)\]", re.IGNORECASE)
    ...
    def _ctx_from_overrun_probe(self, mid):
        ...
        try:
            ...
            # DeepSeek (and possibly others) format the limit differently:
            # "valid range of max_tokens is [1, 393216]"
            m = self._CTX_RANGE_RE.search(msg)
            if m:
                return int(m.group(1))
        return None
...
# --- --prelude / --limbus: limbic system / core philosophy file ---------------
# A markdown/yaml/text file that defines the agent's foundational character,
# values, and behavioral guidelines — separate from the system prompt. Not a
# ruleset, but a "limbic system": the emotional-motivational substrate.
# In one-shot mode (--prompt-file) this content is injected as a grounding user
# message *before* the actual instruction, so the model sees it as context
# without it being part of the system prompt.
_PRELUDE_FILE = None
for _i, _arg in enumerate(sys.argv):
    if _arg in ("--prelude", "--limbus") and _i + 1 < len(sys.argv):
...
# Executes scripts bundled with a skill (under ~/.minion/skills/<skill_name>/).
# The model passes the skill name, the relative script path, and an arg list.
# minion resolves the path, runs the script via python3, and returns stdout.

SKILLS_DIR = os.path.join(SESSION_HOME, "skills")
_SKILL_TIMEOUT_DEFAULT = 120  # seconds, overridable via MINION_SKILL_TIMEOUT


def _get_skill_timeout():
    try:
        return int(os.environ.get("MINION_SKILL_TIMEOUT", str(_SKILL_TIMEOUT_DEFAULT)))
    except ValueError:
        return _SKILL_TIMEOUT_DEFAULT


def execute_skill_script(skill_name, script_path, args=None, **_):
    """Run a skill script.

    The script must exist under ~/.minion/skills/<skill_name>/<script_path>
    (script_path is relative to the skill root, e.g. "scripts/search.py").

    ``args`` is a list of strings — they are passed positionally to the
    script's argv (no shell parsing, no injection risk).
    """
    if not isinstance(args, list):
        args = [str(args)]
    skill_dir = os.path.abspath(os.path.join(SKILLS_DIR, skill_name))
    # Prevent path traversal: skill_dir must be a subdirectory of SKILLS_DIR
    if not skill_dir.startswith(os.path.abspath(SKILLS_DIR)):
        return f"ERROR: invalid skill_name '{skill_name}' (path traversal detected)"
    script_full = os.path.join(skill_dir, script_path)
    if not os.path.isfile(script_full):
        return (f"ERROR: script not found — looked for "
                f"~/.minion/skills/{skill_name}/{script_path}")
    timeout = _get_skill_timeout()
    # Run the script via python3, args as a list (safe, no shell)
    cmd = ["python3", script_full] + args
    try:
        r = subprocess.run(cmd, capture_output=True, text=True, timeout=timeout)
        out = (r.stdout or "") + (r.stderr or "")
        return f"[exit {r.returncode}]\n{out[:200000]}"
    except subprocess.TimeoutExpired:
        return f"ERROR: skill script '{skill_name}/{script_path}' timed out after {timeout}s"
...
TOOLS = [
    ...
    {"type": "function", "function": {"name": "execute_skill_script", "description": "Execute a script from a skill in ~/.minion/skills/<skill_name>/. The script is run via python3 with the provided args as a list. Safe against path traversal and command injection.",
        "parameters": {"type": "object", "properties": {"skill_name": {"type": "string", "description": "The skill directory name under ~/.minion/skills/"}, "script_path": {"type": "string", "description": "Relative path to the script inside the skill folder (e.g. 'scripts/search.py')"}, "args": {"type": "array", "items": {"type": "string"}, "default": [], "description": "Positional arguments passed to the script (list of strings)"}}, "required": ["skill_name", "script_path", "args"]}}},
]
...
def _run_one_shot():
    ...
    # --- load prelude / limbus (core philosophy, before the instruction) -------
    prelude_content = None
    if _PRELUDE_FILE:
        try:
            with open(_PRELUDE_FILE, "r", encoding="utf-8") as f:
                prelude_content = f.read().strip()
        except OSError as e:
            sys.stderr.write(
                f"minion: cannot read --prelude {_PRELUDE_FILE!r}: {e}\n")
            sys.exit(2)
    # --- build message stack ---------------------------------------------------
    # The prelude is injected as a grounding user message BEFORE the actual
    # instruction — it establishes character and values without being part of
    # the system prompt. The model sees the limbic system first, then the task.
    if prelude_content:
        messages = [
            {"role": "system", "content": SYSTEM},
            {"role": "user", "content": prelude_content},
            {"role": "assistant", "content": "Ich habe den Limbus gelesen und verstanden. Ich bin bereit für die Aufgabe."},
